...
export default (props) => {
    let { priority, key, title, projectName, description } = props.data;
    ...
    return (
        <section className={`task task--${priority}`} key={key}>
            ...
            <div className="task__descr">{(props.isDescrOpen) ? description : ""}</div>
            <div className="btns-wrap">
                ...
                <button className="btn" onClick={() => { props.toggleDescription() }}>
                    {(!props.isDescrOpen) ? "Развернуть" : "Свернуть"}
                </button>
            </div>
        </section>
    )
